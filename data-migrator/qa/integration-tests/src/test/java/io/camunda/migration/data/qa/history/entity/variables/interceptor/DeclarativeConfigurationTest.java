...
public class DeclarativeConfigurationTest extends AbstractMigratorTest {
  ...
  public void shouldInvokeConfiguredInterceptorDuringMigration(CapturedOutput output) {
    ...
    ProcessInstance userTaskProcessInstance = runtimeService.startProcessInstanceByKey("userTaskProcessId");
    runtimeService.setVariable(userTaskProcessInstance.getId(), "var", "anotherValue");

    // when running history migration
    historyMigration.getMigrator().migrate();

    // then verify variable transformation
    List<VariableEntity> variables = historyMigration.searchHistoricVariables("var");
    assertThat(variables).hasSize(2);
    assertThat(variables.get(0).value()).isEqualTo("\"transformedValue\"");
    assertThat(variables.get(1).value()).isEqualTo("\"transformedValue\"");
  ...
  public void shouldSkipProcessInstanceDueToExceptionFromInterceptor(CapturedOutput output) {
    ...
    assertThat(output.getOut()).contains(formatMessage(VARIABLE_INTERCEPTOR_FAILED_MSG, ComplexInterceptor.class.getSimpleName(), "exFlag"));
