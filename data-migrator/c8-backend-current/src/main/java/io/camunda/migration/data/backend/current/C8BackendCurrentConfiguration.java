 ...
 */
package io.camunda.migration.data.backend.current;
...
/**
 * Wires the version-specific {@code RdbmsService} @Bean for the "current" C8 minor. The
 * 8.10 constructor added a {@code DeployedResourceDbReader} parameter (and the mapper
 * it depends on), so this backend contributes both that reader and the {@code
 * RdbmsService} itself. The sibling {@code c8-backend-previous} module provides the
 * 8.9 shape.
 */
@Configuration
@Conditional(C8DataSourceConfigured.class)
public class C8BackendCurrentConfiguration implements C8Backend {
