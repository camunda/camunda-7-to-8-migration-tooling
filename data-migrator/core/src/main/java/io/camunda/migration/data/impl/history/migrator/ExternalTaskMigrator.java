...
/**
 * Service class responsible for migrating historic external task log entries from Camunda 7 to
 * Camunda 8.
 * <p>
 * External task logs in Camunda 7 record lifecycle events (creation, failure, success, deletion)
 * for each external task. This migrator converts C7 external task log entries to C8
 * {@link JobDbModel} records, tracking each C7 external task by its external task ID so that only
 * one C8 record is created per C7 external task.
 * </p>
 * <p>
 * The C8 job key is stored in the migration tracking table keyed by the C7 external task ID. This
 * allows the {@link IncidentMigrator} to look up the C8 job key when migrating
    ...
    if (shouldMigrate(c7ExternalTaskId, HISTORY_EXTERNAL_TASK)) {
      ...
      if (processInstance != null) {
        ...
        final Long elementInstanceKey = findFlowNodeInstanceKey(
            c7ExternalTaskLog.getActivityId(), c7ProcessInstanceId);
