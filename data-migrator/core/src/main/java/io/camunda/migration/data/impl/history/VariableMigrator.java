...
public class VariableMigrator extends BaseMigrator<HistoricVariableInstance, VariableDbModel> {
  ...
  public void migrateVariable(HistoricVariableInstance c7Variable) {
    var c7VariableId = c7Variable.getId();
    if (shouldMigrate(c7VariableId, HISTORY_VARIABLE)) {
      ...
      var c7ProcessInstanceId = c7Variable.getProcessInstanceId();
      if (c7ProcessInstanceId != null && isMigrated(c7ProcessInstanceId, HISTORY_PROCESS_INSTANCE)) {
        ...
        var historyCleanupDate = calculateHistoryCleanupDateForChild(processInstance.endDate(), c7Variable.getRemovalTime());
        builder.historyCleanupDate(historyCleanupDate);
      }

      var activityInstanceId = c7Variable.getActivityInstanceId();
      if (isMigrated(activityInstanceId, HISTORY_FLOW_NODE) || isMigrated(activityInstanceId, HISTORY_PROCESS_INSTANCE)) {

        var scopeKey = findScopeKey(activityInstanceId);
        if (scopeKey != null) {
          builder.scopeKey(scopeKey);
        }
      }

      VariableDbModel dbModel;
      try {
        dbModel = convert(c7Variable, builder);
