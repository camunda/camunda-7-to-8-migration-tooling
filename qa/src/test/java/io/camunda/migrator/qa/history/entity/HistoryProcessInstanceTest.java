...
/**
 * Entity test that verifies field mappings for historic process instances.
...
public class HistoryProcessInstanceTest extends HistoryMigrationAbstractTest {
  ...
  protected void verifyProcessInstanceFields(ProcessInstanceEntity processInstance,
    ...
    logs.assertContains("Migration of historic process instance with C7 ID [" + historicProcessInstance.getId() + "] completed");
    ...
    assertThat(processInstance.startDate()).isEqualTo(historicProcessInstance.getStartTime().toInstant().atOffset(ZoneOffset.UTC));
    assertThat(processInstance.endDate()).isEqualTo(historicProcessInstance.getEndTime().toInstant().atOffset(ZoneOffset.UTC));
