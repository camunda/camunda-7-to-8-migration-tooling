...
Use `not applicable` with an out-of-scope decision when no target exists. Use `pending` when a target
exists without an explicit request or when the user declines deployment. Record any alternate binding
or external-deployment plan that resolves a pending deployment.
...
| Generated form structure | Before Step 5e, parse every participating `.form` resource and apply its schema and render checks from `form-migration.md`. Parse every FEEL-bearing form expression with the target FEEL parser when available. Record an unavailable parser in FEEL evidence and continue other required checks without changing the aggregate category state. A failed parse remains a verification failure. Record `not applicable` and the reason when target-compatible schema or render tooling is unavailable, as defined by `form-migration.md`. After Step 5e, verify form linkage. After any form remediation or linkage edit, repeat the JSON, schema, render, and FEEL checks. Run the same full form checks in final verification after deployment. For a standalone `.form` with no owner or converted BPMN, record linkage and deployment as `not applicable` with the reason. Compare the results with the immutable form baseline. Compare `absent` with the created resource when a remediation creates a form. Record before-and-after evidence, the command, exit code, and resource paths. |
...
| Referenced conversion wiring | When code is in scope, derive expected task wiring, listener, header, dispatcher, and DMN/precompute references from each recorded source-to-converted pair. Confirm every expected reference in the converted copy and confirm code coverage, even when the converter emitted no wiring finding. A missing converted reference for an expected source reference is a failed verification unless an explicitly approved replacement or removal is recorded in the finding-specific postcondition and proven by post-remediation XML or code evidence. An unexpected converted reference with no source counterpart is a failed verification unless it is an explicitly approved remediation reference recorded in the category postcondition and post-remediation code evidence, or a reference generated by a recorded converter option such as `--add-data-migration-execution-listener`. Validate approved remediation references against their postconditions and post-remediation code evidence. Validate converter-option-generated references against the recorded option and configured job type. Use the immutable code baseline only for `Before` evidence. Record the row as `not applicable` only when neither side has a wiring reference. In a code-only run, run code-side coverage checks against the recorded worker, listener, dispatcher, and precompute artifacts. Record model-side wiring as `not applicable`. When code is out of scope, confirm matching XML declarations and record code coverage as `not applicable`. |
...
| Converter CSV cell normalization | For `filename` and `elementId`, record the raw CSV cell as evidence, then compare the expected value with its exact CSV encoding. The converter-sanitized encoding prepends one apostrophe when the first character after skipping all `Character.isWhitespace` and `Character.isISOControl` characters is `=`, `+`, `-`, or `@`. When sanitization is a no-op, count the single representation once. If the sanitized and exact encodings are distinct and the identity remains ambiguous, set the comparison to `unavailable` and use non-CSV evidence. |
...
| Supplementary converter or FEEL tooling is unavailable | Evidence-only `unavailable` | Keep `Verification=passed` after the other checks pass. Transition provisional **needs review** to **no action** when no user decision remains, except for `c7-generic-task-form`. Keep that category at **needs review** with its accepted-risk terminal state. Do not write `unavailable` as the aggregate state. | Continue the other required checks. |
...
| **needs review** | `passed` with a completed `c7-generic-task-form` decision | Accepted risk is recorded | Keep the verdict as **needs review**. Do not prompt again or offer the category for remediation. |
