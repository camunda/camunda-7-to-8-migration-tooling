...
- A finished conversion is not a finished migration. Every WARNING, TASK, and REVIEW finding needs
  human follow-up unless its only outstanding action is the shared verification pass and no user
  decision remains.
...
directories, stale migration reports, and other pre-flight leftovers. Add current-run resources
from M1 `Created ...` lines, M2 form-procedure paths, or M3 downloaded acquisition paths. Parse
each resource, record its authoritative path and Modeler metadata, and pair it by form id, retained
reference, or owner evidence. A resource with no owner enters the standalone inventory. Do not rely
only on `.form` paths referenced by BPMN.
...
|---|---|---|
| No target | `target=none`, `request=not applicable`, `authorization=not applicable`, `deployment decision=out of scope`, `deployment=not applicable` | Deployment check satisfied. |
| Target without request | Target, `request=not requested`, authorization state, `deployment=pending` | Obtain request. Keep open. |
| Target with unavailable authorization | Target, request, `authorization=unavailable`, `deployment=pending` | Alternate binding or external plan required before closing. |
| Target with authorized request | Target, request, authorization, `deployment=pending` before final cleanup, deployment result afterward | Verify deployment after final cleanup. |
...
For artifact-producing runs, run an initial verification pass for every category, including INFO and
no-edit categories, before changing its verdict to **no action**. Allow later invalidation and final-integrity reruns after
Step 5e, remediation, linkage, deployment, or final cleanup. Do not start an automatic remediation
retry. Include every category in both the findings inventory and verification table. Use provisional
**needs review** for INFO categories until their verification pass succeeds. Do not request a human
decision for this provisional INFO verdict.
...
decision, then rerun form verification checks.
...
| Referenced wiring additions | An unexpected converted reference with no source counterpart is a failed verification unless it is an explicitly approved remediation reference recorded in the category postcondition or a reference generated by a recorded converter option such as `--add-data-migration-execution-listener`. When code is in scope, validate approved remediation references against post-remediation code evidence. When code is out of scope, use the approved XML and postcondition evidence and record code evidence as `not applicable`. Validate converter-option-generated references against the recorded option and configured job type. |
...
| FEEL syntax | Generate a complete mapping registry with a namespace-aware walker. Emit one registry entry for every eligible location in every participating converted copy, then enrich M1/M3 entries with converter findings and derive M2/E1 entries from the recorded source-to-target pairs. Record source QName/local name, source attribute or text location, target location, expression language, and option/message evidence. Include at least BPMN conditions, timer expressions, loop collections and completion conditions, task scheduling and assignment expressions, job-priority expressions, DMN expression entries, Zeebe input and output `source`, and task-header `value` when the mapping marks them as FEEL. Compare the walker candidate count with the registry count. A missing entry is a failed verification. Parse only registry entries whose language is FEEL. Treat blank BPMN condition language as JUEL unless the recorded converter mapping explicitly changed it to FEEL. Record non-FEEL script values as `not applicable` in their own script category. Extract every participating `.form` template/property expression with the form procedure. Strip one leading `=` serialization marker before parsing and retain the raw value as evidence. Record the parser, expression location, and result for every extracted value, or record `none present` only after the complete registry and walk are exhausted. If no parser is available, record `unavailable` in the FEEL evidence and continue the other required checks. Treat this limitation as non-blocking for the aggregate category state. Keep the category at **needs fix** when parsing fails. |
...
| Converter row identity | Use the recorded category-to-message mapping, not a direct `messageId`-to-category assumption. Compare mapped `messageId`, `severity`, `elementName`, `elementId`, `elementType`, and the resolved normalized `filename` identity with the recorded absolute converted path. For a source-derived category with no converter representation, record `messageId=not applicable` and use its source, model, form, or code evidence instead of filtering it out. |
