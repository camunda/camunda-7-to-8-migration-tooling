...
| Converter CSV candidate selection | Compare the raw and de-sanitized candidates with the expected identity. Use the sole matching candidate. If neither candidate matches or both candidates match distinct identities, set the comparison to `unavailable` and use non-CSV evidence. |
