...
| Referenced conversion wiring | When code is in scope and a remediation or finding references task wiring, listeners, headers, dispatchers, or DMN/precompute wiring, confirm matching declarations and code coverage, even when the converter emitted no wiring finding. Converter output is authoritative for its transformation, not for runtime or code coverage. When code is out of scope, confirm matching XML declarations and record code coverage as `not applicable`. Record the row as `not applicable` when neither the remediation nor the finding references such wiring. |
