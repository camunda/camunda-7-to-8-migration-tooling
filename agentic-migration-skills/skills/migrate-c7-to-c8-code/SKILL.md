...
| Generated form FEEL checks | Extract FEEL expressions from form-js templates and FEEL-capable form properties before parsing. Parse `{{...}}` expressions, `{{#loop}}` expressions, and extracted form-property expressions with the target FEEL parser when available. Do not send raw template delimiters to a plain FEEL parser. Record an unavailable parser in FEEL evidence and continue other required checks without changing the aggregate category state. A failed FEEL parse is a verification failure. Record `none present` when no FEEL expressions exist. |
