...
| Converter CSV cell normalization | For `filename` and `elementId`, compare the expected value with its exact CSV encoding. When the first non-whitespace character is `=`, `+`, `-`, or `@`, also allow the converter-sanitized encoding with one prepended apostrophe. When sanitization is a no-op, count the single representation once. If the sanitized and exact encodings are distinct and the identity remains ambiguous, set the comparison to `unavailable` and use non-CSV evidence. |
