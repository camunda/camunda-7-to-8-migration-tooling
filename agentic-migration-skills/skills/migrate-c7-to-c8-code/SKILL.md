...
| Referenced wiring expectations | When code is in scope, derive expected task wiring, listener, header, dispatcher, and DMN/precompute references from each recorded source-to-converted pair. In a code-only run, use the code-only verification branch and record model-side wiring as `not applicable`. Confirm every expected reference in the converted copy and confirm code coverage, even when the converter emitted no wiring finding. |
