...
Run an initial verification pass for every category, including INFO and no-edit categories, before
changing its verdict to **no action**. Allow later invalidation and final-integrity reruns after
...
| Generated form FEEL checks | Extract FEEL expressions from form-js templates and FEEL-capable form properties before parsing. Parse expressions inside `{{...}}` and `{{#loop}}` headers or bodies with the target FEEL parser when available. Do not send raw `{{#loop}}` delimiters to a plain FEEL parser. Record an unavailable parser in FEEL evidence and continue other required checks without changing the aggregate category state. A failed FEEL parse is a verification failure. Record `none present` when no FEEL expressions exist. |
...
| Generated form final checks | Run the same full form checks in final verification after an authorized deployment. For `target=none` or an accepted external/non-deployment terminal decision, run them after recording that terminal decision; do not require an in-application deployment that is not applicable. Keep the category pending when a selected-target deployment is still pending or declined without an alternate binding or external plan. Compare the results with the immutable form baseline. Compare `absent` with the created resource when a remediation creates a form. |
