...
1. Count tracked changes with
   `git diff --numstat <starting-commit> -- ":(exclude)MIGRATION_REPORT.md"`.
2. List untracked, non-ignored files with
   `git ls-files --others --exclude-standard -z -- ":(exclude)MIGRATION_REPORT.md"`. Count each one
   with `git diff --no-index --numstat -- /dev/null "<path>"`. Git reads `/dev/null` in this command
   as an empty file on every platform. Exit code 1 means that the files differ.
3. Never stage or commit files to count them.
4. List `MIGRATION_REPORT.md` as a report output, not as a change.
