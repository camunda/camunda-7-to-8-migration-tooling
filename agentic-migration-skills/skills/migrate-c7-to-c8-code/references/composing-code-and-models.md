...
- Generated forms with uncovered code consumers or incomplete linkage/deployment: **needs fix**. Pending form or validation decisions: **needs review**. Only accepted and validated forms with covered consumers, complete linkage, and a satisfied deployment check become **no action** after the shared verification pass succeeds. Treat an explicitly justified `not applicable` deployment as satisfying the deployment check when no target exists or deployment is declined. Apply the same exception to a Camunda Form reference. Conversion and relinking are required. Verify deployment only when the user explicitly requests it for a selected target. An explicit out-of-scope decision satisfies the deployment check otherwise.
