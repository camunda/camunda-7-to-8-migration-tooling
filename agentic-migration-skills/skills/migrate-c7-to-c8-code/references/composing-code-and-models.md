...
- **Yes, add/update @Deployment for converted files** (recommended when code scope includes a Spring Boot app) - add or update `@Deployment(resources = ...)` so it targets only converted resources with explicit recursive classpath patterns. For forms, use explicit paths for each form whose Question 7 state authorizes application-startup deployment; do not use a broad `converted-c8-*.form` glob. Never target original diagrams, draft forms, or declined forms. Forms with `target=none`, `pending`, or an external deployment plan remain outside application-startup wiring; leave their deployment wiring deferred and non-deploying.
