...
Apply the category-specific overrides before the fallback. Apply the fallback when a category has no
dedicated cross-check in step 5d and no named form procedure:

| Category condition | Finding severity | Verdict | Cross-reference |
|---|---|---|---|
| `m2-manual-review`, `form-reference-conflict`, or `form-key-unmatched` | Any | needs review | no dedicated cross-check |
| Any other fallback category | INFO | no action | no dedicated cross-check |
| Any other fallback category | REVIEW | needs review | no dedicated cross-check |
| Any other fallback category | WARNING or TASK | needs fix | no dedicated cross-check |
