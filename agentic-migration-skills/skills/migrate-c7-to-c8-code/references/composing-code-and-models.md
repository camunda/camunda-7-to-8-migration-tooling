...
- Generated forms with uncovered code consumers or incomplete linkage/deployment: **needs fix**. Pending form or validation decisions: **needs review**. Only accepted, validated, linked, and deployed forms with covered consumers become **no action** after the shared verification pass succeeds. Record deployment as `not applicable` when no deployment target exists.
