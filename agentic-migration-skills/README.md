...
(`-x`) run needs every decision in the request or configuration. Otherwise, the skill records the
missing choices as open items and stops before dependent changes. `--trust-all-tools` (`-t`)
approves tool requests, not migration decisions.
