...
7. For this Models-only fixture, select no deployment target and record
   ...
   explicitly requested and authorized. If authorization is unavailable, keep
   deployment `pending` unless an alternate binding or external-deployment plan
   is selected. If the user declines deployment for a selected target, keep
   deployment `pending` unless an alternate binding or external-deployment plan
   is selected. Otherwise keep deployment `pending` and do not deploy.
